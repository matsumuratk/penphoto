...
写真を撮り、日本語の手書き風コメントを添えて残すiPhoneアプリです。App Storeで公開しています。
SwiftUI / AVFoundation / Vision / Core Image / PhotoKit。外部パッケージ、APIキー、バックエンドは不要です。

## 入手

[App Store: PenPhoto](https://apps.apple.com/jp/app/penphoto/id6811245656)
...
- 写真の90度回転を左右どちらにも実行可能。「ことば」画面上部と「写真・ビューティー」タブの両方から操作でき、文字入力中も使える（取り消し・やり直し対応）
- 明るさ、左右90度回転、1:1 / 4:5 / 16:9 / 自由な比率でのトリミング。ドラッグで位置、ピンチで範囲を調整し、リセット・確定・キャンセルに対応
...
App Storeでの公開は完了しています。Apple純正カメラ相当の全機能を備えた版ではなく、以下は今後の対応です。
...
`PenPhotoTests`: 日本語フォント、編集情報の往復、文字の画素合成、左右の回転、位置・範囲を指定したトリミングとその境界、自由な比率、回転との組み合わせ、旧形式データの読み込み、プレビューと書き出しの一致、顔なし時の処理、元写真を保持した再保存を検証。
`PenPhotoUITests`: サンプルを開き、コメント編集・下書き保存・再編集・写真への書き出しに加え、写真の左右回転（画面上部と写真タブの両方、取り消し・やり直しを含む）、トリミング位置調整のドラッグ・ピンチ操作、取り消し・やり直し、再編集時の保持を検証。
...
単体テスト29件・UIテスト5件が成功しています（2026-09-23、失敗0）。単体テストはシミュレータ（iPhone 17 Pro）と実機（iPhone 12 mini）の両方で、UIテストはシミュレータで実行しました。
画面と確認範囲は [検証記録](docs/VERIFICATION.md) に記載しています。
